...
void *readtx(void *arg) {
	...
	if (currentTx->status == TR_ACTIVE) {
		currentTx->set_lock(node->tid, currentTx->sgno, node->obno, node->count,
				'S');
	}
	finish_operation(node->tid);
	pthread_exit(NULL);				// thread exit
...
void *do_commit_abort(long t, char status) {
	...
	if (currentTx != NULL) {
		...
		zgt_v(0);
		for (int i = 0; i < zgt_nwait(t); i++) {
			zgt_v(currentTx->semno);
		}
		zgt_p(0);
		currentTx->remove_tx();
		zgt_v(0);
		open_logfile_for_append();
		if (status == TR_END) {
			fprintf(logfile, "T%d\t%c \tCommmitTx\n", currentTx->tid, currentTx->Txtype);// Write log record and close
			fflush(logfile);
		...
		printf(":::ERROR:node with tid:%d does not exists", t);
		fflush (stdout);
	}
	// write your code
}
...
int zgt_tx::set_lock(long tid1, long sgno1, long obno1, int count,
	...
	zgt_hlink *link = ZGT_Ht->findt(tid1, sgno1, obno1);
	if (link != NULL) {
		this->status = TR_ACTIVE;
		this->perform_readWrite(tid1,obno1,lockmode1);
	} else {
		link = others_lock(NULL, sgno1, obno1);
		if (link == NULL) {
			//No one has locked this object
			zgt_p(0);
			this->status = TR_ACTIVE;
			...
			//object is locked by another transaction
			zgt_tx *transaction = get_tx(link->tid);
			if (transaction->Txtype == 'R' && lockmode1 == 'S'
					&& zgt_nwait(transaction->tid) == 0 && this->Txtype == 'R') {
				//Allow other transaction to read object locked by Read-Only Transaction
				zgt_p(0);
				this->status = TR_ACTIVE;
				ZGT_Ht->add(this, sgno1, obno1, lockmode1);
				zgt_v(0);
				...
				if (transaction->semno == -1) {
					transaction->semno = transaction->tid;
				}
				this->status = TR_WAIT;
				this->obno = obno1;
				setTx_semno(transaction->tid, transaction->semno);
				wait_for * waitgraph = new wait_for();
				long abort_tid= waitgraph->deadlock();
				if(abort_tid == this->tid){
					do_commit_abort(this->tid,TR_ABORT);
				}else{
					if(abort_tid>0){
						do_commit_abort(abort_tid,TR_ABORT);
					}else{
						zgt_p(transaction->semno);
					}
	...
	//	zgt_hlink *link = this->others_lock(NULL,sgno1,obno1);
	//	  if(link != NULL){
	//		  if(link->tid != tid1){
	//			  zgt_tx * transaction = get_tx(link->tid);
	//			  if(transaction->Txtype =='R' && lockmode == 'S'){
	//				  //object is locked by read only transaction with different tid
	//			  }
	//			  else{
	//				  //object is locked by read/write transaction with different tid.
	//				  setTx_semno(link->tid,link->tid);
	//				  this->status =TR_WAIT;
	//				  this->obno =obno1;
	//				  zgt_p(link->tid);
	//
	...
	//		  }
	//		  else{
	//			  //object is already locked by the same transaction. Do nothing.
	//		  }
	//	  }
	//	  else{
	//		  ZGT_Ht->add(this,sgno1,obno1,lockmode1);
...
int zgt_tx::free_locks() {
	...
	for (temp; temp != NULL; temp = temp->nextp) {	// SCAN Tx obj list
		...
		fprintf(logfile, "%d : %d \t", temp->obno,
				ZGT_Sh->objarray[temp->obno]->value);
		fflush(logfile);

		if (ZGT_Ht->remove(this, 1, (long) temp->obno) == 1) {
			printf(
					":::ERROR:node with tid:%d and objno:%d was not found for deleting",
